    ...
    <div className="p-4 md:p-8 pb-24 md:pb-8">
      ...
      >
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-100">Keuangan</h1>
            <p className="text-sm text-slate-400">Informasi tagihan & pembayaran</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 w-full lg:w-auto">
          <div className="relative flex-1 lg:min-w-[250px]">
            <select
              ...
              className="w-full appearance-none bg-slate-900/80 border border-slate-700/50 text-slate-200 text-sm rounded-xl py-3 pl-4 pr-10 focus:outline-none focus:ring-2 focus:ring-orange-500/50"
            >
          ...
          <button 
            onClick={fetchData} 
            disabled={loading}
            className="p-3 bg-slate-800/80 rounded-xl hover:bg-slate-700 transition-all hover:scale-105 active:scale-95 disabled:opacity-50 border border-slate-700/50"
          >
            <RefreshCw className={`w-5 h-5 text-slate-300 ${loading ? 'animate-spin text-orange-400' : ''}`} />
          </button>
        </div>
      </motion.header>
      ...
      {!loading && tagihan.length > 0 && (
        ...
        >
          <div className="glass-dark rounded-3xl p-6 border border-white/5 relative overflow-hidden">
            <div className="absolute top-0 right-0 w-32 h-32 bg-blue-500/10 rounded-full blur-2xl -translate-y-1/2 translate-x-1/2" />
            <p className="text-sm font-medium text-slate-400 mb-2">Total Tagihan</p>
            <p className="text-2xl font-bold text-white">{formatRupiah(totalTagihan)}</p>
          </div>
          <div className="glass-dark rounded-3xl p-6 border border-white/5 relative overflow-hidden">
            <div className="absolute top-0 right-0 w-32 h-32 bg-emerald-500/10 rounded-full blur-2xl -translate-y-1/2 translate-x-1/2" />
            <p className="text-sm font-medium text-slate-400 mb-2">Sudah Dibayar</p>
            <p className="text-2xl font-bold text-emerald-400">{formatRupiah(totalDibayar)}</p>
          </div>
          <div className="glass-dark rounded-3xl p-6 border border-white/5 relative overflow-hidden">
            <div className="absolute top-0 right-0 w-32 h-32 bg-orange-500/10 rounded-full blur-2xl -translate-y-1/2 translate-x-1/2" />
            <p className="text-sm font-medium text-slate-400 mb-2">Sisa Tagihan</p>
            <p className="text-2xl font-bold text-orange-400">{formatRupiah(totalSisa)}</p>
          </div>
        </motion.div>
      )}
      ...
      {loading ? (
        <div className="glass-dark rounded-3xl p-16 flex flex-col items-center justify-center space-y-4 border border-white/5">
          <div className="w-12 h-12 rounded-full border-4 border-slate-700 border-t-orange-500 animate-spin" />
          <p className="text-slate-400 font-medium">Memuat data keuangan...</p>
        </div>
      ) : tagihan.length === 0 ? (
        <div className="glass-dark rounded-3xl p-16 text-center border border-white/5">
          ...
          <h3 className="text-lg font-semibold text-slate-300 mb-1">Data tidak ditemukan</h3>
          <p className="text-slate-500 text-sm">Tidak ada riwayat tagihan untuk semester ini.</p>
        </div>
      ) : (
        <div className="space-y-4">
          <h2 className="text-xl font-bold text-white px-2 mb-4">Rincian Tagihan</h2>
          <AnimatePresence>
            {tagihan.map((item, i) => (
              <motion.div
                key={i}
                ...
                className="glass-dark rounded-2xl p-6 border-l-4 border-l-orange-500 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 hover:bg-slate-900/60 transition-colors"
              >
                <div className="flex-1">
                  <h3 className="text-lg font-bold text-white mb-2">{item.nama_tagihan || 'Tagihan Semester'}</h3>
                ...
                <div className="flex flex-wrap md:flex-nowrap items-center gap-6 md:gap-12 w-full md:w-auto bg-slate-950/50 md:bg-transparent p-4 md:p-0 rounded-xl md:rounded-none border md:border-none border-slate-800/50">
                  <div className="flex flex-col">
                    <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider mb-1">Biaya</span>
                    <span className="text-sm font-bold text-slate-300">{formatRupiah(item.jumlah)}</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider mb-1">Dibayar</span>
                    <span className="text-sm font-bold text-emerald-400">{formatRupiah(item.sudah_dibayar)}</span>
                  </div>
                  <div className="flex flex-col">
                    <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider mb-1">Sisa</span>
                    <span className="text-sm font-bold text-orange-400">{formatRupiah(item.sisa_tagihan)}</span>
                  </div>
                </div>
              </motion.div>
            ))}
